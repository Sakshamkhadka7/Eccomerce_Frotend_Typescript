...
import { cancelOrder, fetchMyOrdersDetails } from "../../store/checkoutSlice";
    ...
    <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
      <div className="flex justify-start item-start space-y-2 flex-col">
        ...
        <h1 className="text-xl dark:text-white lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">
          Order Status {orderDetails[0]?.Order.orderStaus}
        </h1>
        <p className="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">
          Data {new Date(orderDetails[0]?.createdAt).toLocaleDateString()}
        </p>
      </div>
                    ...
                    <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                      ...
                      <div className="flex justify-between space-x-8 items-start w-full">
                        ...
                        <p className="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">
                          {od?.quantity * od.Product.productPrice}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
            ...
            <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
              ...
              <div className="flex w-full justify-center items-center md:justify-start md:items-start">
                {orderDetails[0]?.Order?.orderStaus !== OrderStaus.Cancelled && (
                  <button
                    onClick={cancelOrderByUser}
                    className="mt-6 md:mt-0 bg-red-600 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base font-medium leading-4 text-gray-800"
                  >
                    Cancel Order
                  </button>
                )}
